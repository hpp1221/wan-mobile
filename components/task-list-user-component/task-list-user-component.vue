		...
		<block v-for="(value, index) in listData" :key="value.id">
			<view class="uni-list-cell" @click="goDetail(value.id)">
				<view class="uni-media-list">
					<view class="img-wrap">
						<image class="uni-media-list-logo" :src="value.cover_pic_url"></image>
						<view class="remian-time">
							<text class="num-info-time">剩余时间</text>
							<uni-countdown color="#FFFFFF" splitorColor="#FFFFFF" :day="value.day" :hour="value.hour" :minute="value.min" :second="value.sec" class="num"></uni-countdown>
						</view>
					</view>
					<view class="uni-media-list-body">
						<view class="task-name ellipsis-1">{{ value.name }}</view>
						<view class="task-title ellipsis-1">等待上传证明截图</view>
...
<style lang="less">
@import url('./task-list-user-component.less');
</style>
